...
- Added normative section "Declaration format" to SPEC.md. Defines the `/.well-known/ai-posture.json` artifact, required and optional fields (`type`, `spec_version`, `generated_at`, `next_review`, `subject`, `assertion_basis`, `aggregate`, `constraining_vectors`, `vectors`, optional `evidence` and per-vector `posterior`), the three `assertion_basis` values (`self-estimate`, `self-assertion`, `verified`), and stale-declaration semantics.
- `verified` is reserved; no verification process is defined in v1.1.

Version alignment:

- Bumped `package.json` 1.0.0 to 1.1.0 and SPEC.md frontmatter v1.0.0 to v1.1.0 (`last_modified` 2026-06-10).
- Published JSON Schema at `docs/schema/declaration/v1/ai-posture-declaration.schema.json` (canonical: https://aiposture.org/schema/declaration/v1/ai-posture-declaration.schema.json).
- Updated `docs/.well-known/ai-posture-framework.json` to add `declaration` block and advance `spec.version` to v1.1.0.
- Updated `docs/llms.txt` spec version reference and added Declaration Schema entry.
- Added Papers nav link to the spec page (generated by `scripts/build-spec.js`).
- ROADMAP Item 10.5 (declaration format): complete.

Wire format (ROADMAP Item 12, EARL):

- Published the AI Posture EARL v0 profile at `docs/schema/earl/v0/`: `context.jsonld`, `shapes.ttl`, `profile.md`, `example.jsonld`. W3C EARL 1.0 vocabulary with `apos:` extension terms; OSCAL projection (Item 13) stays deferred.
- Minted stable criterion IRIs in the `https://aiposture.org/criteria/v1/` namespace (one per vector per level 1-5, 15 total); registry at `docs/criteria/v1/index.json`, generated from `rubric.json`.
- Shipped the zero-dependency reference validator `scripts/validate.js` (MIT) and a worked declaration example at `docs/schema/declaration/v1/example.json`; both wired into `npm test` (`tests/earl-profile.test.js`).
- Extended `docs/.well-known/ai-posture-framework.json` agent resources, `docs/llms.txt`, and `docs/sitemap.xml` to advertise the declaration schema, declaration example, criterion registry, EARL profile, and the `/check/` viewer.
- Proposed Obligation-First linkage for Regulation-vector criteria recorded in `profile.md` as non-normative, pending steward review.
- ROADMAP Item 12 (EARL wire format): complete.

Surfaces:

- Added the declaration viewer at `docs/check/` and the adjacent-framework crosswalk page at `docs/crosswalk/`.
- Refreshed the social card `docs/imgs/og.png` to the vendor-neutral positioning and cache-busted `og:image` references.
